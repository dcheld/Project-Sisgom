...
@WebServlet(urlPatterns = "/fornecedor/Editar")
public class FornecedorEditarServlet extends HttpServlet {

	private static final long serialVersionUID = 1L;
	private static final Logger LOGGER = Logger.getLogger(FornecedorEditarServlet.class);
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (entity != null) {
				...
				resp.sendRedirect(String.format("/fornecedor/list.jsp?error=Fornecedor [%s] não foi encontrado.", id));
			}
		} catch (Exception ex) {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			FornecedorEntity entity = this.parseToCliente(req);
